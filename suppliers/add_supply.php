...
<ul>
    ...
    <li><a class="active" href="manage_supply.php">Orders</a>
        <ul>
            <li><a href="manage_supply.php">Manage</a></li>
            <li><a class="active" href="add_supply.php">Add</a></li>
        </ul>
    </li>
...
<div class="cuerpo">
    ...
    function myFunction() {
        if (nu <= '40') {
            ...
            io3.setAttribute('id', 'cod_mat'+nu);
            io3.setAttribute('name', 'cod_mat'+nu);
    ...
    <form enctype="multipart/form-data" action="" method="post">
        Supply code: <input type="number" name="num" value="<?php 
            $orde = mysql_query('SELECT MAX(cod_sup) AS cod_sup FROM upply_m_s');
        ...
        <!--Supplier: 
        <select name="sup1" onchange="changeSup(this)" style="white-space:pre-wrap; width: 100px;" required>
            <option selected="selected"></option>
            <?php
            $sqlcli = "SELECT * FROM suppliers";
            $clis = mysql_query($sqlcli);
            while ($rowcli = mysql_fetch_assoc($clis)) {
                print("<option value='".$rowcli[cod_sup]."'>$rowcli[name]</option>");
            }
            ?>
        </select> Code: <input id="cod_sup" type="number" name="cod_sup" Style="width:60Px" value="" disabled="TRUE"/><br><br>-->
        ...
        Materials: <button type="button" onclick="myFunction()">Add one more</button>
        <table id="myTable">
            <tr>
                <td><label>Material:</label> 
                <select name="material1" onchange="changeMat(this,1)" style="white-space:pre-wrap; width: 250px;">
                    <option selected="selected"></option>
                    <?php
                    $sql = "SELECT * FROM materials";
                    $cons = mysql_query($sql);
                    while ($row = mysql_fetch_assoc($cons)) {
                        print("<option value='".$row[cod_mat]."'>$row[description]</option>");
                    }
                    ?>
                </select> Code: <input id="cod_mat1" type="number" name="cod_mat1" Style="width:60Px" value="" disabled="TRUE"/></td>
