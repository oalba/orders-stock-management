...
while ($row5 = mysql_fetch_assoc($selec)) {
    echo "
    <tr bgcolor=\"#c1bda8\" align=center>
        <th>Order code</th>
        <th>Client</th>
        <th>Manager</th>
        <th>Order date</th>
    ...
    <tr bgcolor=#dddddd>
        <td rowspan=2>
            <input type=\"number\" name=\"cod_ord\" value=\"$row5[cod_ord]\" style=\"white-space:pre-wrap; width: 80px;\" disabled=\"disabled\"/>
        </td>
        <td>
            <select name=\"cli1\" onchange=\"changeCli(this)\" style=\"white-space:pre-wrap; width: 100px;\" disabled=\"disabled\" required>";
            ...
            <br/>
            Code: <input id=\"cod_cli\" type=\"number\" name=\"cod_cli\" Style=\"width:60Px\" value=\"$row5[cod_cli]\" disabled=\"TRUE\"/>
        </td>
        <td>
            <select name=\"man1\" onchange=\"changeMan(this)\" style=\"white-space:pre-wrap; width: 100px;\" disabled=\"disabled\">";
            ...
            <br/>
            Code: <input id=\"cod_man\" type=\"number\" name=\"cod_man\" Style=\"width:60Px\" value=\"$row5[cod_man]\" disabled=\"TRUE\"/>
        </td>
        <td>
            <input type=\"date\" name=\"order_date\" value=\"$row5[order_date]\" disabled=\"disabled\"/>
        </td>
        <td>";
            if ($row5['deliver_date'] != NULL) {
                echo "
                <input type=\"date\" name=\"deliver_date\" value=\"$row5[deliver_date]\" disabled=\"disabled\"/>";
            } else {
                echo "
                <input type=\"date\" name=\"deliver_date\" value=\"\" disabled=\"disabled\"/>";
            }
        echo "
        </td>
        <td>
            <select name=\"delivered\" style=\"white-space:pre-wrap; width: 60px;\" disabled=\"disabled\">";
                if ($row5['delivered']=='0') {
                    echo "
        ...
        <td>
            <textarea name=\"remarks\" rows=\"3\" cols=\"50\" disabled=\"disabled\">$row5[remarks]</textarea>
        </td>
        <td>
            <input type=\"number\" name=\"discount\" step=\"any\" Style=\"width:60Px\" value=\"$row5[discount]\"/ disabled=\"disabled\">%
        </td>
        <td>
            <a href='edit_orders.php?cod_ord=$data' style=\"text-decoration:none\">
                <input type='button' value='Edit' class='button1'/>
            </a>
        </td>
        <td>
            <button onclick=\"seguroord($row5[cod_ord]);\" class=\"button1\">Delete</button>
        </td>
    </tr>
    <tr>
        <td colspan='9'>
            <table border='1' width='100%' bgcolor=#dddddd>
                ...
                while ($row2 = mysql_fetch_assoc($cons2)) {
                    if ($data2!=$row2['cod_piece']) {
                        echo "
                        <tr>
                            <td>
                                <input type=\"number\" name=\"quantity0\" value=\"$row2[quantity]\" Style=\"width:40Px\" disabled=\"disabled\"/>
                            </td>
                            <td>
                                <select name=\"cod_piece0\" onchange=\"changePie(this,10)\" style=\"white-space:pre-wrap; width: 250px;\" disabled=\"disabled\">";
                                ...
                                Code: <input id=\"cod_piece10\" type=\"number\" name=\"cod_piece10\" Style=\"width:60Px\" value=\"$row2[cod_piece]\" disabled=\"disabled\"/>
                            </td>";
                            ...
                            while ($row3 = mysql_fetch_assoc($cons3)) {
                                ...
                                if (isset($row3['plans']) and $row3['plans']!=""){
                                    echo "
                                    <a href=\"plans/$row3[plans]\" style=\"text-decoration:none\">
                                        <input type=\"button\" value=\"Download plans\" class=\"button1\">
                                    </a>";
                                } else {
                            ...
                            echo "
                            <td>
                                <textarea name=\"remarks10\" rows=\"1\" cols=\"50\" disabled=\"disabled\">$row2[remarks]</textarea>
                            </td>
                            <td>
                                <a href='edit_p_o.php?cod_ord=$data&cod_piece=$row2[cod_piece]' style=\"text-decoration:none\">
                                    <input type='button' value='Edit' class='button1'/>
                                </a>
                            </td>
                            <td>
                                <button onclick=\"seguroOrdPie('".$row2['cod_piece']."',".$data.");\" class='button1'>Delete</button>
                            </td>
                        </tr>";
                    } else {
                        echo "
                        <tr>
                        <form enctype='multipart/form-data' action='inc/edit_p_o_act.php?cod_ord=$data&cod_piece=$data2' method='post'>
                            <td>
                                <input type=\"number\" name=\"quantity\" value=\"$row2[quantity]\" Style=\"width:40Px\"/>
                            </td>
                            <td>
                                <select name=\"cod_piece\" onchange=\"changePie(this,1)\" style=\"white-space:pre-wrap; width: 250px;\" disabled=\"disabled\">";
                                $sql = "SELECT * FROM pieces";
                                $cons = mysql_query($sql);
                                while ($row = mysql_fetch_assoc($cons)) {
                                    if ($row2['cod_piece']==$row['cod_piece']) {
                                ...
                                Code: <input id=\"cod_piece1\" type=\"number\" name=\"cod_piece1\" Style=\"width:60Px\" value=\"$row2[cod_piece]\" disabled=\"disabled\"/>
                            </td>";
                            $sql3 = "SELECT * FROM pieces WHERE cod_piece=$row2[cod_piece]";
                            $cons3 = mysql_query($sql3);
                            while ($row3 = mysql_fetch_assoc($cons3)) {
                            ...
                            echo "
                            <td>
                                <textarea name=\"remarks1\" rows=\"1\" cols=\"50\">$row2[remarks]</textarea>
                            </td>
                            <td colspan=2>
                ...
                echo "
                <tr>
                <form enctype='multipart/form-data' action='inc/add_order_pie_act.php?cod_ord=$data' method='post'>
                    <td>
                        <input type=\"number\" name=\"cant0\" value=\"1\" Style=\"width:40Px\"/>
                    </td>
                    <td>
                        <select name=\"piece0\" onchange=\"changePie(this,0)\" style=\"white-space:pre-wrap; width: 250px;\">
                        ...
                        echo "
                        </select> Code: <input id=\"cod_piece0\" type=\"number\" name=\"cod_piece0\" Style=\"width:60Px\" value=\"\" disabled=\"disabled\"/>
                    </td>
                    <td>
                    </td>
                    <td>
                        <textarea name=\"remarks0\" rows=\"1\" cols=\"50\"></textarea>
                    </td>
                    <td colspan=2>
                        <input type='submit' name='save' value='Save' class='button1'/>
                    </td>
