...
while ($row = mysql_fetch_assoc($phones)) {
    ...
    echo "<td><input size=\"20\" type=\"file\" name=\"plans\" disabled=\"disabled\"/><br/>";
    if (isset($row['plans']) and $row['plans']!=""){
    ...
    echo "</td>";
    echo "<td><textarea name='description' rows='3' cols='50' disabled=\"disabled\">$row[description]</textarea></td>";
    echo "<td><input type='number' name='price' step='any' Style='width:60Px' value='$row[price]' disabled=\"disabled\"> ¥</td>";
    echo "<td><input type='number' name='stock' Style='width:60Px' value='$row[stock]' disabled=\"disabled\"></td>";
	echo "<td><a href='edit_pieces.php?cod_piece=$data' style=\"text-decoration:none\"><input type='button' value='Edit' class='button1'/></a></td>";
    ...
    while ($row2 = mysql_fetch_assoc($cons2)) {
        if ($data2!=$row2['cod_mat']) {
            ...
            echo "<form enctype='multipart/form-data' action='inc/edit_m_p_act.php?cod_piece=$data&cod_mat=$data2' method='post'>";
            echo "<tr>
            <td><input type=\"number\" name=\"quantity\" value=\"$row2[quantity]\" Style=\"width:40Px\"/></td>";
            echo "<td><select name=\"cod_mat\" onchange=\"changeMat(this,1)\" style=\"white-space:pre-wrap; width: 250px;\" disabled=\"disabled\">";
            ...
            echo "<td><textarea name=\"remarks1\" rows=\"1\" cols=\"50\">$row2[remarks]</textarea></td>";
            /*echo "<td>editar</td><td>eliminar</td></tr>";
            echo "<td><button onclick=\"window.location.href='edit_orders.php?cod_ord=$row5[cod_ord]'\" class=\"button1\">Edit</button></td>";
            echo "<td><button onclick=\"seguroord($row5[cod_ord]);\" class=\"button1\">Delete</button></td>";*/
            echo "<td colspan=2><input type='submit' name='save' value='Save' class='button1'/></td>";
            //echo "<td><button onclick=\"seguroPieMat('".$row2['cod_mat']."',".$data.");\" class='button1'>Delete</button></td>";
            echo "</tr></form>";
        }
    }
    echo "<tr>";
