...
<ul>
    ...
    <li><a class="active" href="manage_clients.php">Clients</a>
        <ul>
            <li><a class="active" href="manage_clients.php">Manage</a></li>
...
<div class="cuerpo">
	<h1><u><i>Manage clients</i></u></h1>
<form enctype="multipart/form-data" action="" method="post">
	Add data: <br/>
	<textarea name="data" rows="3" cols="50" placeholder="Client code, Name, Phone or Details... (Do not add anything to show all)"></textarea><br/><br/>
	Order by: 
	<select name="orden">
		<option value="cod_cli" selected>Client code</option>
...
$sql = "SELECT * FROM clients WHERE cod_cli LIKE '%$data%' OR name LIKE '%$data%' OR details LIKE '%$data%' OR phone LIKE '%$data%' ORDER BY $orden";
...
echo "NOTE: If the client has orders, it won't be deleted. <br/> $zenbat clients in total.";

$num_fila = 0; 
echo "<table border=1>";
echo "<tr bgcolor=\"bbbbbb\" align=center><th>Client code</th><th>Name</th><th>Phone</th><th>Details</th><th colspan=2>Actions</th></tr>";
while ($row = mysql_fetch_assoc($conce)) {
	...
	echo "<td><button onclick=\"window.location.href='edit_clients.php?cod_cli=$row[cod_cli]'\" class='button1'>Edit</button></td>";
	echo "<td><button onclick=\"segurocli('$row[cod_cli]');\" class='button1'>Delete</button></td>";
	echo "</tr>";
	$num_fila++; 
};
echo "</table><br/>";
echo "Is it not here? <a href='add_clients.php' style=\"text-decoration:none\"><input type='button' value='Add'></a>";
